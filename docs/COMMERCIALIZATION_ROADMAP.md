...
- **Dependency/secret scanning in CI** — `.github/dependabot.yml` covers `pip`/`docker`/`github-actions`; a `security` job in `.github/workflows/ci.yml` runs bandit (SAST), `pip-audit` against `requirements-lock.txt`, and gitleaks, and gates `docker-build` on all three passing. Closed the Pillar 4 pre-GA blocker. (Also fixed the one real bandit finding it surfaced: `ingestion/extractors/structured.py`'s XML parsing now goes through `defusedxml` instead of stdlib `xml.etree.ElementTree`, since that module processes ingested, untrusted content.)
- **Org/workspace entity** — `Organization` (`storage/models/orgs.py`) plus `users.org_id` exist; `POST /v1/orgs`, `GET /v1/orgs/me`, `POST /v1/orgs/invite`, `GET /v1/orgs/members` are live (`api/routers/orgs.py`), and `accounts.create_invite` threads an optional `org_id` through so org invites join the invited user to that org on acceptance.
- **Org-scoped/seat-counted billing** — `Subscription.org_id` (nullable, unique when set) lets a per-seat plan (`Team`) be bought once by an org's owner and cover every member: `governance.billing.get_subscription` checks for an org-scoped row before falling back to the caller's own personal one, checkout quantity is the org's live member count, and only the owner can check out or manage the shared subscription's Stripe portal (a real gap caught in review: any member could otherwise open the org's billing portal). A member with no org-wide plan still keeps their own personal Free/Pro subscription unaffected.
- **Web frontend: org/team management** — `app.html`'s settings modal now has an Organization section wired to the `/v1/orgs/*` API added alongside the entity itself: create one, see its member list and invite teammates (owner view), or just see who owns it (member view). The billing section shows "shared with your organization" and hides the manage-billing control for non-owners, matching the server-side owner-only enforcement above.
- **Marketing site: hosted vs. self-host, explicitly** — `index.html` gained a dedicated Deployment section presenting both as first-class, equally-supported paths (not self-host-is-an-Enterprise-afterthought), and `pricing.html`'s `Team` tier now says "Hosted or self-hosted" instead of only `Enterprise` mentioning the option.
- **Per-customer dedicated-instance provisioning (Path A)** — `scripts/provision_customer.sh <slug> <domain> --admin-email <email>` scripts the whole manual `.env`/secrets/`docker compose up`/migrate/create-admin-user sequence into one command, with a non-colliding host-port block per stack (`docker-compose.yml`'s Postgres/Neo4j/MinIO/api host ports are now parameterized, defaults unchanged) so more than one customer stack can run on one host; `scripts/deprovision_customer.sh <slug>` tears one back down. See README's Production readiness section. Still manual: DNS, and deploying `web/`'s static frontend (no compose service serves it).
- **Per-plan rate limiting** — `POST /v1/query`'s rate limit is now resolved per-caller from their actual plan (`common/plans.py`'s new `query_rate_limit_per_minute`), not one flat number for every key regardless of tier. Closes the specific gap called out as "the one thing standing between a free-tier key and a real bill once `LLM_CLIENT_MODE=live`." Scoped to `AUTH_MODE=api_key`; `AUTH_MODE=oidc` callers still get the flat default (documented as a follow-up, not silently dropped).
- **Encrypted conversation vault (Pillar 2)** — a solo Pro-tier user's query history is encrypted at rest under a per-user, password-derived key (`security/vault.py`); recovery codes (now issued unconditionally at signup, not just on opting into MFA — closes a real Pillar-1 gap found along the way) double as an escrow secret. Team/Enterprise's shared, plaintext, org-wide audit log — a real selling point for compliance-minded teams — is untouched, matching Pillar 5's own tier table. A genuinely separate bug found and fixed in the process: `GET /v1/session/{id}` had no ownership check at all before this.
- **Post-quantum audit-export signing (Pillar 3.3)** — each Iceberg export of the audit log gets a small ML-DSA-signed receipt (`storage/iceberg_signing.py`), independently verifiable without trusting this deployment's current key. `cryptography` (already a dependency) ships native ML-KEM/ML-DSA support — no `liboqs`, no new sidecar service needed.
- **Post-quantum TLS in transit (Pillar 3.1)** — documented, not application code: see `docs/DEPLOYMENT_PQC.md`. A CDN/WAF in front (Cloudflare or equivalent) already gets hybrid X25519+ML-KEM-768 for free; self-terminating Caddy TLS is still waiting on upstream Go/Caddy hybrid-KEM support.
...
- **Stripe is unverified against a live account.**
- **Pillar 3.2 (hybrid PQC key wrapping) deliberately not built** — see the Phase 2 line below for why: the vault's actual crypto chain (Argon2id KDF → AES-256-GCM → HKDF session bridge) is entirely symmetric, with no RSA/ECC/DH exchange anywhere in it for "harvest now, decrypt later" to threaten. Wrapping a key that was never asymmetrically exchanged in the first place would be a security claim without a matching mechanism — exactly what this doc's own "quantum encryption" section warns against. Revisit if/when a genuinely asymmetric exchange (e.g. WebAuthn/passkey-based escrow) gets added to the vault design.

**Net effect on the phased roadmap below**: Phase 0 is functionally done. Phase 1 is essentially done (billing plumbing, the org/workspace entity, org-scoped/seat-counted billing, its web UI, hosted-vs-self-host marketing packaging, per-plan rate limiting, and per-customer provisioning all exist). Phase 2 is done except Pillar 3.2 (deliberately deferred, see above). Phases 3-4 are unchanged and still fully open — legal entity formation, a live Stripe verification, a pen test, SOC 2, and case studies all need real-world action this repo can't take on its own. The Ops Console effort tracked separately (`web/ops.html` + graph/jobs/audit/terminal admin tooling — see the active plan file) is Phase 3/4-adjacent: it's internal support/ops tooling, not customer-facing, but it becomes more valuable exactly when there are paying customers' connector jobs and audit trails to actually watch — sequence it opportunistically, not as a blocker.
...
~~Add `password_hash`/MFA columns to `User`; signup/login/recovery routes; web signup+login pages in front of `app.html`~~ **done** (see status update above). ~~Dependency/secret scanning in CI~~ **done**. Still open from this phase: legal entity + ToS/Privacy Policy drafted with counsel.

**Phase 1 — Path A commercial MVP (4-8 weeks)**
~~Stripe billing + plan gating~~ **billing plumbing done** (`POST /checkout`/`/portal`/`/webhook` live; a live, non-mock Stripe account still needs verifying end-to-end — everything else in this phase is code-complete). ~~Org/seats as a real entity~~ **done**, including org-scoped/seat-counted billing and its web UI (`POST /v1/orgs` + `users.org_id` + `Subscription.org_id` — see status update above). ~~Per-customer dedicated-instance provisioning~~ **done** (`scripts/provision_customer.sh`/`deprovision_customer.sh`). ~~Hosted vs. self-host packaging on the marketing site~~ **done** (`index.html`'s Deployment section, `pricing.html`'s Team tier).

**Phase 2 — Privacy & crypto (4-6 weeks, can run parallel to Phase 1)**
~~Pillar 2's per-user encrypted conversation vault; recovery-code escrow~~ **done** (`security/vault.py`, `governance/accounts.py` — see status update above). ~~ML-DSA signing of the audit export (Pillar 3.3)~~ **done** (`storage/iceberg_signing.py`). ~~CDN/WAF in front for PQC-in-transit "for free" (Pillar 3.1)~~ **documented** (`docs/DEPLOYMENT_PQC.md`) — an infra/deployment choice, not application code, so "done" here means the guidance exists, not that a CDN is sitting in front of anything by default. **Hybrid PQC key wrapping (Pillar 3.2) deliberately not built** — see the status update above: the vault's actual crypto is entirely symmetric, nothing asymmetric exists yet for a hybrid KEM to protect. Revisit only if the vault design later grows a real asymmetric exchange.

**Phase 3 — Trust & scale (ongoing from ~month 3)**
Pen test → SOC 2 Type I (needs a real external auditor/pentester engaged — outside this repo's own reach); tenant-scoped audit log queries once any shared-deployment tier exists; begin the Path B multi-tenant migration *only* once a specific customer or cost pressure justifies it — Postgres RLS first, Neo4j isolation strategy decided at that point based on actual tenant count and Enterprise-license economics at the time.

**Phase 4 — Growth**
SOC 2 Type II; expand SSO/IdP coverage beyond Keycloak (Okta/Auth0/Entra ID, currently untested per the README); case studies from the legal/journalism/diligence beachhead (needs actual paying customers willing to go on record); evaluate QKD only if a specific customer's threat model and budget genuinely calls for point-to-point dedicated-fiber links — realistically not before this product has enterprise customers asking for it by name.
