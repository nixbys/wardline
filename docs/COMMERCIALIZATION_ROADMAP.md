...
~~Stripe billing + plan gating~~ **billing plumbing done** (`POST /checkout`/`/portal`/`/webhook` live; plan-gating enforcement and a live, non-mock Stripe account still need verifying end-to-end). ~~Org/seats as a real entity~~ **done** (`POST /v1/orgs` + `users.org_id`; billing itself is still per-user, not org-scoped — see status update above). Still open: per-customer dedicated-instance provisioning (scripted `docker compose` stand-up, not manual); hosted vs. self-host packaging live on the marketing site.
