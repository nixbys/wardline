...
- **Org/workspace entity** — `Organization` (`storage/models/orgs.py`) plus `users.org_id` exist; `POST /v1/orgs`, `GET /v1/orgs/me`, `POST /v1/orgs/invite`, `GET /v1/orgs/members` are live (`api/routers/orgs.py`), and `accounts.create_invite` threads an optional `org_id` through so org invites join the invited user to that org on acceptance. Deliberately narrow: `Subscription` is still keyed to `user_id`, not `org_id` — org-scoped/seat-counted billing (Pillar 5's Team/Enterprise tiers) is real follow-on work now that the grouping entity exists to hang it off of, not done here.
...
- **Billing isn't org-scoped yet** — the org/workspace entity above exists, but `Subscription` is still one-per-`user_id` (unique constraint), and the `Team` plan's `per_seat` flag is still cosmetic (rendered in plan copy, not enforced anywhere) — see the org/workspace bullet above for the deliberate scope boundary.
...
~~Stripe billing + plan gating~~ **billing plumbing done** (`POST /checkout`/`/portal`/`/webhook` live; plan-gating enforcement and a live, non-mock Stripe account still need verifying end-to-end). ~~Org/seats as a real entity~~ **done** (`POST /v1/orgs` + `users.org_id`; billing itself is still per-user, not org-scoped — see status update above). ~~Per-customer dedicated-instance provisioning~~ **done** (`scripts/provision_customer.sh`/`deprovision_customer.sh` — see status update above). Still open: hosted vs. self-host packaging live on the marketing site.
