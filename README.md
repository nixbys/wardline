...
- **Deploying to Vercel instead of docker-compose**: possible, but not a drop-in target — see [`docs/VERCEL_DEPLOYMENT.md`](docs/VERCEL_DEPLOYMENT.md) for the full story (bundle-size fix, `api/routers/cron.py` standing in for the persistent worker process, managed Postgres/Neo4j/object-storage wiring, and the Hobby-vs-Pro Cron cadence tradeoff that directly affects background-job timeliness).
